...
ENV=dev NETWORK=testnet CHAIN=BTC API_PREFIX=http://127.0.0.1:8200/api npm -prefix=./packages/insight run ionic:serve --nobrowser & npm -prefix=./packages/bitcore-node run start